...
namespace PersonHub.Api.Areas.LifeEvents.Controllers
{
    ...
    public class EventsController : ApiControllerBase
    {
        ...
        {
            var eventEntity = await dbContext.Events.FirstOrDefaultAsync(r => r.Id == id);
        ...
        public async Task<ActionResult<IEnumerable<Event>>> Query(QueryEventRequestDto dto, CancellationToken cancellationToken)
        {
            ...
            var result = await dbContext.Events.OrderByDescending(r => r.EventDate).Skip(dto.Offset).Take(dto.Limit).ToListAsync();
            return result;
        }
        ...
        public async Task<ActionResult<Event>> AddEvent(EventRequestDto dto, CancellationToken cancellationToken)
        {
            ...
            var eventEntity = new Event(dto.Title, dto.Description, dto.EventDate.Value, dto.Tags);
